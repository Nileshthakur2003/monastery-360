...
export default function EncheyMonasteryPage() {
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false)

  // Use the provided embed for the virtual tour section
  const virtualTourEmbed = (
    <iframe 
      src="https://www.google.com/maps/embed?pb=!4v1759126169240!6m8!1m7!1sCAoSF0NJSE0wb2dLRUlDQWdJQ0pzTXEzOWdF!2m2!1d27.33593677395685!2d88.61916587167339!3f44.015686!4f0!5f0.7820865974627469" 
      width="100%" 
      height="100%" 
      style={{ border: 0 }} 
      allowFullScreen
      ...
      title="Enchey Monastery Virtual Tour"
    ></iframe>
  )
    ...
    <main className={`min-h-screen ${primaryBg} ${primaryText} font-sans`}>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden border-t bg-white">
            <nav className="flex flex-col gap-3 px-4 py-4 text-sm">
              ...
              <a href="/explore/enchey" className={`${accentColorClass} font-semibold`}>Enchey Monastery</a>
      ...
      <section className="max-w-6xl mx-auto px-4 py-10 md:py-16 text-center">
        ...
        <p className="mt-3 text-slate-600 max-w-xl mx-auto text-sm md:text-base">
          Discover the 'Solitary Temple,' a 200-year-old sanctuary blessed by a flying tantric master, nestled amidst the serene pine forests near Gangtok.
        </p>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-16 grid gap-8 md:grid-cols-2">
        ...
        <div className={`rounded-2xl border ${borderColorClass} p-6 bg-slate-50/50`}>
          <h3 className={`text-base md:text-lg font-semibold ${accentColorClass}`}>About Enchey Monastery</h3>
          <p className="text-sm text-slate-700 mt-3">
            Dating back around 200 years, Enchey Monastery belongs to the Nyingma order. It is an important spiritual center in Gangtok, known for its serene atmosphere and traditional architecture adorned with bright murals of Buddhist deities and symbols.
          </p>
        </div>
        {/* Visitor Info */}
        <div className={`rounded-2xl border ${borderColorClass} p-6 bg-slate-50/50`}>
          ...
          <ul className="mt-3 text-sm text-slate-700 space-y-2">
            <li>Location: 3 km northeast of Gangtok center, East Sikkim</li>
            <li>Order: Nyingma (Red Hat) sect of Vajrayana Buddhism</li>
            <li>Highlights: Rare collection of masks, Cham Dance during the **Pang Lhabsol festival** (typically September).</li>
            <li>Etiquette: Respectful attire is required inside the main prayer hall.</li>
          </ul>
        </div>
      </section>
