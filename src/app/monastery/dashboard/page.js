        ...
        case 'Overview':
            return (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    <StatCard icon={Users} title="Total Monks" value="25" />
                    <StatCard icon={Calendar} title="Upcoming Events" value="3" />
                    <StatCard icon={Mail} title="Messages" value="12" change="+2" />
                </div>
            );
        case 'Events':
             return (
                <ContentBlock title="Events & Festivals" icon={Calendar}>
                    <div className="bg-slate-50 p-3 rounded-md border">Buddha Jayanti – May 2025</div>
                    <div className="bg-slate-50 p-3 rounded-md border">Annual Monastic Debate – Aug 2025</div>
                    <div className="bg-slate-50 p-3 rounded-md border">Prayer Festival – Dec 2025</div>
                </ContentBlock>
            );
        case 'Monks':
             return (
                <ContentBlock title="Monks Directory" icon={Users}>
                    <div className="bg-slate-50 p-3 rounded-md border">Ven. Tashi Lama – Head Monk</div>
                    <div className="bg-slate-50 p-3 rounded-md border">Ven. Karma Dorjee – Senior Monk</div>
                    <div className="bg-slate-50 p-3 rounded-md border">Ven. Sonam – Junior Monk</div>
                </ContentBlock>
            );
        case 'Gallery':
             return (
                <ContentBlock title="Gallery" icon={Image}>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <div className="w-full h-32 bg-gray-200 rounded-lg shadow animate-pulse"></div>
                        <div className="w-full h-32 bg-gray-200 rounded-lg shadow animate-pulse"></div>
                        <div className="w-full h-32 bg-gray-200 rounded-lg shadow animate-pulse"></div>
                        <div className="w-full h-32 bg-gray-200 rounded-lg shadow animate-pulse"></div>
                    </div>
        ...
        case 'Messages':
             return (
                <ContentBlock title="Messages" icon={Mail}>
                    <p className="text-slate-600">No new messages yet.</p>
                </ContentBlock>
            );
        default:
      ...
      <div className="flex">
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex-col flex transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="flex-1 px-4 py-2">
            <h2 className="px-4 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">Monastery Management</h2>
