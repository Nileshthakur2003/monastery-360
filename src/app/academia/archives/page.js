...
  ];
  ...
  const renderContent = () => {
    switch (activeMenu) {
            ...
            return (
                <div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <StatCard icon={Archive} title="Total Items" value="1,204" change="+32" />
                        <StatCard icon={Upload} title="New Submissions" value="15" />
                        <StatCard icon={Users} title="Active Volunteers" value="28" />
                    </div>
                    <div className="mt-8">
                       <ContentBlock title="Featured Items" icon={Archive}>
                         <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-4">
                            {archives.map((item) => (
                              <div
                                key={item.id}
                                className="rounded-lg border border-slate-200 p-5 hover:shadow-sm transition bg-slate-50"
                              >
                                <h4 className="font-semibold text-base">{item.title}</h4>
                                <p className="text-sm text-slate-600 mt-2">{item.desc}</p>
                                <div className="mt-4 flex flex-wrap gap-2 text-sm">
                                  <button className="px-3 py-1 rounded-full border bg-white hover:border-emerald-500">
                                    View
                                  </button>
        ...
        case 'Search':
             return (
                <ContentBlock title="Search the Archive" icon={Search}>
                    ...
                    <div className="flex gap-3 flex-col sm:flex-row">
                      ...
                      />
                      <button className="rounded-full bg-emerald-500 text-white px-6 py-2 hover:bg-emerald-600 text-sm font-semibold">
                        Search
                      </button>
                    </div>
                </ContentBlock>
        ...
        case 'Community':
             return (
                <ContentBlock title="Community Involvement" icon={Users}>
                     <div className="rounded-lg border border-slate-200 p-6 bg-slate-50">
                        ...
                        <button className="mt-4 w-full sm:w-auto rounded-full border px-4 py-2 hover:border-emerald-500 bg-white text-sm font-semibold">
                          Become a Volunteer
                        </button>
                      </div>
                </ContentBlock>
        ...
        case 'Uploads':
             return (
                <ContentBlock title="Community Contributions" icon={Upload}>
      ...
      <div className="flex">
        <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-slate-200 flex-col flex transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          ...
          <nav className="flex-1 px-4 py-2">
            <h2 className="px-4 py-2 text-xs font-semibold text-slate-400 uppercase tracking-wider">Archives Dashboard</h2>
          ...
          <div className="p-4 border-t border-slate-200">
              <div className="flex items-center gap-3">
                  <img src="https://placehold.co/40x40/E2E8F0/475569?text=A" alt="Manager" className="w-10 h-10 rounded-full" />
                  <div>
                      <p className="font-semibold text-sm">Archive Manager</p>
                      <p className="text-xs text-slate-500">archive@email.com</p>
                  </div>
              </div>
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                  ...
                  <button className="p-2 rounded-full hover:bg-slate-100">
                   ...
                   <button className="flex items-center gap-2 p-2 rounded-full hover:bg-slate-100">
                      <img src="https://placehold.co/32x32/E2E8F0/475569?text=A" alt="Manager" className="w-8 h-8 rounded-full" />
